...
function ChangePasswordForm() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try { await setPassword(password); router.replace(next); } catch (caught) { setError(caught instanceof Error ? caught.message : "Passordet kunne ikke endres"); setLoading(false); }
  }
  return <main className="grid min-h-screen place-items-center p-5 sm:p-10"><div className="w-full max-w-md rounded-[28px] border border-[var(--line)] bg-[var(--surface)] p-8 shadow-[var(--shadow)]"><Logo /><span className="mt-8 grid h-12 w-12 place-items-center rounded-2xl bg-[var(--mint)]">{forced ? <ShieldCheck size={25} /> : <KeyRound size={24} />}</span><h1 className="mt-6 text-3xl font-black tracking-[-.045em]">{forced ? "Velg passord" : "Endre passord"}</h1><p className="mt-3 leading-7 text-[var(--ink-soft)]">{forced ? `Kontoen din ble opprettet med et midlertidig passord. Velg ditt eget for å fortsette — minst ${MIN_PASSWORD_LENGTH} tegn.` : `Velg et nytt passord på minst ${MIN_PASSWORD_LENGTH} tegn.`}</p>{joining && <p className="mt-3 rounded-xl bg-[var(--mint)] px-3 py-2 text-sm font-semibold">Etterpå går du rett videre til laginvitasjonen.</p>}<form className="mt-7 grid gap-5" onSubmit={submit}><Field label="Nytt passord"><input type="password" required autoComplete="new-password" minLength={MIN_PASSWORD_LENGTH} className={inputClass} value={password} onChange={(event) => setNewPassword(event.target.value)} autoFocus /></Field><Field label="Bekreft nytt passord"><input type="password" required autoComplete="new-password" className={inputClass} value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></Field>{error && <p role="alert" className="rounded-xl bg-red-50 px-3 py-2 text-sm font-semibold text-[var(--danger)]">{error}</p>}<Button size="lg" disabled={loading}>{loading ? "Lagrer…" : "Lagre passord"}</Button></form></div></main>;
}
