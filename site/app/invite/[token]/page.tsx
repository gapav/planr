...
export default function InvitePage() {
  const { token } = useParams<{ token: string }>(); const { user, isDemoMode, refreshWorkspace } = useGrep(); const [remoteStatus, setRemoteStatus] = useState<"loading" | "accepted" | "alreadyJoined" | "error">("loading"); const [remoteMessage, setRemoteMessage] = useState("Godtar laginvitasjonen …"); const attempted = useRef<string | null>(null);
  ...
  useEffect(() => { if (!user || isDemoMode || attempted.current === token) return; attempted.current = token; const supabase = getSupabaseBrowserClient(); if (!supabase) return; void supabase.rpc("accept_team_invitation", { invitation_token: token }).then(async ({ error }) => { if (error) { if (isInvitationAlreadyUsed(error.message)) { await refreshWorkspace(); setRemoteStatus("alreadyJoined"); return; } setRemoteStatus("error"); setRemoteMessage(norwegianServerMessage(error.message, "Invitasjonen kunne ikke godtas.")); return; } await refreshWorkspace(); setRemoteStatus("accepted"); }); }, [isDemoMode, refreshWorkspace, token, user]);
