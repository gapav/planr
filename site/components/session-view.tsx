...
import { ArrowLeft, Calendar, CirclePlay, Clock3, Eye, LayoutList, MapPin, Pencil, Printer, Sparkles, Target } from "lucide-react";
...
import { ItemThumbnailPlaceholder } from "./exercise-thumbnail";
...
export function SessionView({ sessionId, readOnly = false }: { sessionId: string; readOnly?: boolean }) {
  ...
  return <AppShell><div data-print="sheet" className="min-h-screen pb-24">
    ...
    <div data-print="body" className="mx-auto max-w-[1000px] px-4 pt-7 sm:px-7">
      ...
      <section data-print="plan" className="mt-8">
        ...
        {session.blocks.length ? <div data-print="blocks" className="mt-6 grid gap-5">{session.blocks.map((block, index) => { const stations = isStationBlock(block); const clock = blockClock(schedule, block.id); return <article key={block.id} data-print="block" className="overflow-hidden rounded-[24px] border border-[var(--line)] bg-[var(--surface)] shadow-[0_7px_24px_rgba(16,32,29,.04)]">
          <header data-print="block-head" className="flex items-center gap-3 border-b border-[var(--line)] p-4 sm:px-5">{clock ? <span className="shrink-0 rounded-xl bg-[var(--paper-deep)] px-2 py-1.5 text-xs font-black tabular-nums">{clockTime(clock.startsAt)}</span> : <span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-[var(--paper-deep)] text-xs font-black">{index + 1}</span>}{blockGlyph(block)}<h3 className="min-w-0 flex-1 truncate text-lg font-black">{block.title}</h3>{stations && <span className="hidden shrink-0 rounded-full bg-[var(--paper-deep)] px-2.5 py-1 text-[11px] font-black sm:inline-block">{block.items.length} × {stationRotation(block)} min</span>}<span className="shrink-0 rounded-full bg-black/5 px-2.5 py-1 text-xs font-black">{blockDuration(block)} min</span></header>
          ...
          {block.notes && <BlockNote notes={block.notes} />}
          <div className="grid gap-2.5 p-3 sm:p-4">{block.items.length ? block.items.map((item, itemIndex) => <button key={item.id} type="button" data-print="item" onClick={() => setPreviewItem(item)} aria-label={`Vis ${item.title}`} className="group flex items-start gap-3 rounded-2xl border border-[var(--line)] bg-white p-3 text-left transition hover:border-[var(--ink)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--orange)]">
            <span className="relative h-11 w-11 shrink-0 overflow-hidden rounded-xl sm:h-12 sm:w-12">{item.thumbnailUrl ? <img src={item.thumbnailUrl} alt="" className="h-full w-full object-cover" /> : <ItemThumbnailPlaceholder item={item} />}<span className="absolute inset-0 grid place-items-center bg-black/25 opacity-0 transition group-hover:opacity-100"><span className="grid h-7 w-7 place-items-center rounded-full bg-white/90"><Eye size={14} /></span></span>{stations && <span className="absolute left-0 top-0 grid h-5 w-5 place-items-center rounded-br-lg bg-[var(--ink)] text-[10px] font-black text-white">{itemIndex + 1}</span>}</span>
