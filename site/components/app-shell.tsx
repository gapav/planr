...
export function AppShell({ children, publicPage = false, immersive = false }: { children: React.ReactNode; publicPage?: boolean; immersive?: boolean }) {
  ...
  return <div className={cn("min-h-screen lg:grid lg:transition-[grid-template-columns] lg:duration-200", sidebarCollapsed ? "lg:grid-cols-[76px_minmax(0,1fr)]" : "lg:grid-cols-[252px_minmax(0,1fr)]")}>
    <aside id="app-sidebar" className={cn("fixed inset-y-0 left-0 z-40 flex w-[280px] flex-col border-r border-white/10 bg-[var(--ink)] p-4 text-white transition-transform lg:sticky lg:top-0 lg:h-screen lg:w-auto lg:translate-x-0", mobileOpen ? "translate-x-0" : "-translate-x-full", sidebarCollapsed && "lg:px-3")}>
      ...
      <div className={cn("mt-7 rounded-2xl border border-white/10 bg-white/[.07] p-2", sidebarCollapsed && "lg:hidden")}><span className="px-2 text-[10px] font-black uppercase tracking-[.14em] text-white/45">Lag</span><div className="relative mt-1"><select value={currentTeam?.id ?? ""} onChange={(event) => setCurrentTeamId(event.target.value)} className="min-h-12 w-full appearance-none rounded-xl bg-transparent px-2 pr-8 text-sm font-bold text-white outline-none">{teams.map((team) => <option key={team.id} value={team.id} className="text-black">{team.shortName}</option>)}</select><ChevronDown className="pointer-events-none absolute right-2 top-3.5 text-white/50" size={17} /></div></div>
