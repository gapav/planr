...
const genericPlaceholder = { emoji: "📋", background: "bg-[var(--paper-deep)]" };
...
export function ExerciseThumbnail({ exercise, className }: { exercise: ThumbnailSubject; className?: string }) {
  ...
  if (failed || !exercise.thumbnailUrl) {
    ...
    return <div className={cn("grid place-items-center overflow-hidden", placeholder.background, className)}><span className="text-[clamp(2rem,7vw,5rem)] leading-none" role="img" aria-label={exercise.category ? `${exercise.category}-øvelse` : "Aktivitet"}>{placeholder.emoji}</span></div>;
  }
