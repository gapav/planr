...
export function ExerciseThumbnail({ exercise, className }: { exercise: Exercise; className?: string }) {
  ...
  if (failed || !exercise.thumbnailUrl) {
    ...
    return <div className={cn("grid place-items-center overflow-hidden", placeholder.background, className)}><span className="text-[clamp(2rem,7vw,5rem)] leading-none" role="img" aria-label={`${exercise.category}-øvelse`}>{placeholder.emoji}</span></div>;
  }
