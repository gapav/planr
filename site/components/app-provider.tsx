...
function demoAdminAccounts(): AdminAccount[] {
  return sortAdminAccounts(demoProfiles.map((profile) => ({
    id: profile.id,
    email: profile.email,
    fullName: profile.fullName,
    initials: profile.initials,
    isGlobalAdmin: profile.isGlobalAdmin === true,
    createdAt: "2026-08-20T08:00:00.000Z",
    lastSignInAt: profile.id === "user-sam" ? null : "2026-09-12T08:00:00.000Z",
    filesOwned: 0,
    memberships: demoTeams
      .filter((team) => team.members.some((member) => member.id === profile.id))
      .map((team) => ({ teamId: team.id, teamName: team.name, teamRole: team.members.find((member) => member.id === profile.id)?.teamRole ?? "coach" })),
  })));
}
...
interface GrepContextValue {
  ...
  /**
   * Every samling for the coach's teams — named shortlists over the library,
   * belonging to the team rather than to whoever started one. Consumers scope
   * to the team they mean with `collectionsForTeam`.
   */
  ...
  /** Starts a samling on the current team and hands back its id, so the caller can fill it at once. */
  createCollection(name: string): Promise<string>;
  renameCollection(id: string, name: string): Promise<void>;
  deleteCollection(id: string): Promise<void>;
  toggleCollectionExercise(collectionId: string, exerciseId: string): Promise<void>;
  /** The signed-in coach's own opt-out for the morning session email. */
  setSessionDigestEmail(enabled: boolean): Promise<void>;
  /** Renames the signed-in coach everywhere they are shown. Login stays the email. */
  ...
  /** A system administrator renames a coach whose name is still the email stub. */
...
function profileFromUser(user: User): Profile {
  ...
  return { id: user.id, email: user.email ?? "", fullName, initials: initials(fullName), color: COACH_AVATAR_SELF };
}
...
function mapCollection(row: DbCollection): ExerciseCollection {
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [collections, setCollections] = useState<ExerciseCollection[]>(() => isSupabaseConfigured ? [] : structuredClone(demoCollections));
  ...
  const loadAdminAccounts = useCallback(async () => {
    ...
    if (error) setNotice(norwegianServerMessage(error.message, "Brukerkontoene kunne ikke lastes."));
    else setAdminAccounts(sortAdminAccounts(((data ?? []) as AdminAccountRow[]).map(mapAdminAccount)));
  ...
  const loadPrivateData = useCallback(async (authUser: User) => {
    ...
    const [{ data: memberships }, { data: sessionRows }, { data: invitationRows }, { data: profileRow, error: profileError }, { data: playerRows }, { data: fixtureRows }, { data: monthFocusRows }, { data: warmupRows }, { data: attendanceRows }, { data: groupingRows }, { data: favoriteRows }, { data: collectionRows }] = await Promise.all([
      ...
      // Embedded rather than a second round trip: a samling is its name and its
    ...
    if (memberships) {
      ...
      for (const membership of memberships as unknown as MembershipRow[]) {
        ...
        if (membership.profiles && !existing.members.some((member) => member.id === membership.profile_id)) {
          existing.members.push({ id: membership.profiles.id, email: membership.profiles.email, fullName: membership.profiles.full_name, initials: initials(membership.profiles.full_name), color: membership.profile_id === authUser.id ? COACH_AVATAR_SELF : COACH_AVATAR_PEER, teamRole: membership.role });
        }
    ...
    // A missing table — the migration not yet applied by hand — leaves this
    // null, which reads as "no samlinger" rather than taking the workspace down.
    if (collectionRows) setCollections(sortCollections((collectionRows as unknown as DbCollection[]).map(mapCollection)));
    if (profileRow?.is_global_admin) await Promise.all([loadAdminTeams(), loadAdminAccounts()]);
    else {
      setAdminTeams([]);
      setAdminAccounts([]);
      setAdminTeamsLoaded(true);
  ...
  const signOut = useCallback(async () => {
    ...
    // Samlinger are the team's, and the library stays readable signed out, so
    // the rail would otherwise keep offering them to whoever sits down next.
  ...
  // Samlinger are the other half of the same idea and deliberately not the same
  // table: a heart is private to the coach, a samling belongs to the team, so
  // these write through `is_team_member` and every coach on the team sees the
  // result. All four follow the house pattern — optimistic state, `persist`,
  // roll back on refusal — because a name or a tick that survives a failure is
  // a lie the coach only finds out about on the next load.
  const createCollection = useCallback(async (name: string) => {
    if (!currentTeam) throw new Error("Velg et lag først");
    if (!user) throw new Error("Logg inn for å lage samlinger");
    const teamId = currentTeam.id;
    const trimmed = normalizeCollectionName(name);
    const invalid = collectionNameError(trimmed, collections.filter((entry) => entry.teamId === teamId));
    if (invalid) throw new Error(invalid);
    const id = makeUuid();
    const now = new Date().toISOString();
    setCollections((current) => sortCollections([...current, { id, teamId, name: trimmed, exerciseIds: [], createdBy: user.id, createdAt: now, updatedAt: now }]));
    try {
      await persist(supabase ? () => supabase.from("exercise_collections").insert({ id, team_id: teamId, name: trimmed, created_by: user.id }) : null);
    } catch (error) {
      setCollections((current) => current.filter((entry) => entry.id !== id));
      throw error;
    }
    return id;
  }, [collections, currentTeam, persist, supabase, user]);

  const renameCollection = useCallback(async (id: string, name: string) => {
    const existing = collections.find((entry) => entry.id === id);
    if (!existing) return;
    const trimmed = normalizeCollectionName(name);
    const invalid = collectionNameError(trimmed, collections.filter((entry) => entry.teamId === existing.teamId), id);
    if (invalid) throw new Error(invalid);
    if (trimmed === existing.name) return;
    const previous = collections;
    setCollections((current) => sortCollections(current.map((entry) => entry.id === id ? { ...entry, name: trimmed } : entry)));
    try {
      await persist(supabase ? () => supabase.from("exercise_collections").update({ name: trimmed }).eq("id", id) : null);
    } catch (error) {
      setCollections(previous);
      throw error;
    }
  }, [collections, persist, supabase]);

  // The membership rows go with it: the cascade in 202609170001 is what keeps
  // this one delete rather than two.
  const deleteCollection = useCallback(async (id: string) => {
    const previous = collections;
    setCollections((current) => current.filter((entry) => entry.id !== id));
    try {
      await persist(supabase ? () => supabase.from("exercise_collections").delete().eq("id", id) : null);
    } catch (error) {
      setCollections(previous);
      throw error;
    }
  ...
  // Membership is a set keyed by (samling, øvelse), so an add that races
  // another coach's add is the same row rather than a duplicate-key refusal —
  // hence `ignoreDuplicates` instead of a plain insert.
  const toggleCollectionExercise = useCallback(async (collectionId: string, exerciseId: string) => {
    const collection = collections.find((entry) => entry.id === collectionId);
    if (!collection) return;
    const wasIn = collection.exerciseIds.includes(exerciseId);
    const apply = (inList: boolean) => (current: ExerciseCollection[]) => current.map((entry) => entry.id !== collectionId ? entry : {
      ...entry,
      exerciseIds: inList
        ? entry.exerciseIds.includes(exerciseId) ? entry.exerciseIds : [...entry.exerciseIds, exerciseId]
        : entry.exerciseIds.filter((id) => id !== exerciseId),
    });
    setCollections(apply(!wasIn));
    try {
      await persist(supabase ? () => wasIn
        ? supabase.from("exercise_collection_items").delete().eq("collection_id", collectionId).eq("exercise_id", exerciseId)
        : supabase.from("exercise_collection_items").upsert({ collection_id: collectionId, exercise_id: exerciseId }, { onConflict: "collection_id,exercise_id", ignoreDuplicates: true }) : null);
    } catch (error) {
      setCollections(apply(wasIn));
      throw error;
    }
  ...
  // The screen name. `full_name` starts as the email local part because the
  // account is created before anyone asks the coach what to call them, and it
  // is what avatars, bylines and the morning letter show — so the rename has to
  // reach the copies the workspace already holds: the profile itself, the
  // coach's row in every team they are on, and the byline on exercises they
  // own. Nothing about signing in changes; the email is still the login.
  const setDisplayName = useCallback(async (name: string) => {
    if (!user) throw new Error("Logg inn for å endre visningsnavnet");
    const fullName = normalizeDisplayName(name);
    const refusal = displayNameError(fullName);
    if (refusal) throw new Error(refusal);
    if (fullName === user.fullName) return;
    const previous = user;
    const rename = <T extends { id: string; fullName: string; initials: string }>(profile: T): T =>
      profile.id === user.id ? { ...profile, fullName, initials: initials(fullName) } : profile;
    setUser((current) => current && { ...current, fullName, initials: initials(fullName) });
    setTeams((current) => current.map((team) => team.members.some((member) => member.id === user.id) ? { ...team, members: team.members.map(rename) } : team));
    setExercises((current) => current.map((exercise) => exercise.createdBy === user.id ? { ...exercise, createdByName: fullName } : exercise));
    try {
      await persist(supabase ? () => supabase.from("profiles").update({ full_name: fullName }).eq("id", user.id) : null);
    } catch (error) {
      setUser((current) => current && { ...current, fullName: previous.fullName, initials: previous.initials });
      setTeams((current) => current.map((team) => ({ ...team, members: team.members.map((member) => member.id === user.id ? { ...member, fullName: previous.fullName, initials: previous.initials } : member) })));
      setExercises((current) => current.map((exercise) => exercise.createdBy === user.id ? { ...exercise, createdByName: previous.fullName } : exercise));
      throw error;
    }
  }, [persist, supabase, user]);
  ...
  const createTeam = useCallback(async (name: string, firstAdminEmail?: string) => {
    ...
    return id;
  }, [loadAdminAccounts, refreshWorkspace, sendAuthLinkEmail, supabase, user]);
  ...
  const adminInviteMember = useCallback(async (teamId: string, email: string, role: TeamRole): Promise<InviteResult> => {
    ...
    if (user.isGlobalAdmin) await loadAdminAccounts();
    return { inviteUrl: invitationUrl(window.location.origin, token), loginLink, emailed: emailError === null, emailError };
  }, [loadAdminAccounts, patchAdminTeam, persist, sendAuthLinkEmail, syncOwnTeams, supabase, user]);
  ...
  const adminSetMemberRole = useCallback(async (teamId: string, profileId: string, role: TeamRole) => {
    ...
    setAdminAccounts((current) => current.map((account) => account.id === profileId ? {
      ...account,
      memberships: account.memberships.map((membership) => membership.teamId === teamId ? { ...membership, teamRole: role } : membership),
    } : account));
    await syncOwnTeams(teamId);
  }, [adminTeams, patchAdminTeam, persist, supabase, syncOwnTeams]);

  // An account is created from the dashboard, so `full_name` starts as the email
  // local part and the administrator who onboards a coach is the one looking at
  // it. `admin_set_display_name` re-checks `is_global_admin` in the database and
  // writes that one column: the table-wide column grant means a policy would
  // have handed over the coach's personal digest opt-out as well.
  const adminSetDisplayName = useCallback(async (profileId: string, name: string) => {
    const fullName = normalizeDisplayName(name);
    const refusal = displayNameError(fullName);
    if (refusal) throw new Error(refusal);
    const previousTeams = adminTeams;
    const previousAccounts = adminAccounts;
    const rename = <T extends { id: string; fullName: string; initials: string }>(profile: T): T =>
      profile.id === profileId ? { ...profile, fullName, initials: initials(fullName) } : profile;
    setAdminTeams((current) => sortAdminTeams(current.map((team) => ({ ...team, members: team.members.map(rename) }))));
    setAdminAccounts((current) => sortAdminAccounts(current.map(rename)));
    try {
      await persist(supabase ? () => supabase.rpc("admin_set_display_name", { profile_id: profileId, display_name: fullName }) : null);
    } catch (error) {
      setAdminTeams(previousTeams);
      setAdminAccounts(previousAccounts);
      throw error;
    }
    // The renamed coach may be the administrator themselves, or sit on one of
    // their own teams, where the old name is still in the sidebar and on bylines.
    if (profileId === user?.id || teams.some((team) => team.members.some((member) => member.id === profileId))) await refreshWorkspace();
  }, [adminAccounts, adminTeams, persist, refreshWorkspace, supabase, teams, user]);
  ...
  const deleteAccountPermanently = useCallback(async (profileId: string, confirmationEmail: string) => {
    if (!user?.isGlobalAdmin) throw new Error("Bare en systemadministrator kan slette en konto permanent.");
    if (profileId === user.id) throw new Error("Du kan ikke slette din egen systemadministratorkonto.");
    ...
    if (supabase) {
      ...
      const response = await fetch("/api/admin/accounts", {
        method: "DELETE",
        headers: { Authorization: `Bearer ${data.session.access_token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ profileId, confirmationEmail }),
      });
      const payload = await response.json().catch((): { error?: string } => ({}));
      if (!response.ok) throw new Error(payload.error ?? "Kontoen kunne ikke slettes permanent.");
    }

    setAdminAccounts((current) => current.filter((account) => account.id !== profileId));
    setAdminTeams((current) => current.map((team) => ({
      ...team,
      members: team.members.filter((member) => member.id !== profileId),
    })));
