 ...
 * invent six unrelated metaphors for what is really one axis.
 */
const agePresentation: Record<ExerciseAgeGroup, { selected: string; idle: string }> = {
  "6-9": { selected: "border-[#3f8f6b] bg-[#3f8f6b] text-white", idle: "border-[#c3e3d1] bg-[#eef8f2] text-[#2f6a4f] hover:border-[#8ac2a4]" },
  "10-12": { selected: "border-[#357052] bg-[#357052] text-white", idle: "border-[#b6d9c5] bg-[#e8f4ed] text-[#2a5d43] hover:border-[#79b498]" },
  "13-15": { selected: "border-[#27553d] bg-[#27553d] text-white", idle: "border-[#a8d0b9] bg-[#e2f0e8] text-[#234f39] hover:border-[#6aa88c]" },
};
...
export function ExerciseAgeGroupFilter({ value, onChange }: { value: ExerciseAgeGroup | null; onChange(ageGroup: ExerciseAgeGroup | null): void }) {
  const options: Array<{ label: string; value: ExerciseAgeGroup | null; selected: string; idle: string }> = [
    { label: "Alle aldre", value: null, selected: "border-[var(--ink)] bg-[var(--ink)] text-white", idle: "border-[var(--line)] bg-white text-[var(--ink-soft)] hover:border-[var(--ink-soft)]" },
