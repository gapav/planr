...
function SessionSetup({ sessionId }: { sessionId: string }) {
  ...
  // A stations block only works if there is one group per station, so the draw
  // opens on the number the plan already implies rather than on a bare two.
  const suggestedGroups = suggestedGroupCount(session);
  const [teamCount, setTeamCount] = useState(suggestedGroups ?? 2);
  ...
  const settingsCard = <div className="rounded-[28px] border border-[var(--line)] bg-[var(--surface)] p-5 shadow-[0_10px_35px_rgba(16,32,29,.05)] sm:p-6">
    ...
    {groupingKind === "teams" && <label className="mt-6 block text-sm font-black"><span>Antall lag</span><div className="mt-2 flex items-center rounded-2xl border border-[var(--line)] bg-white p-1"><button type="button" className="grid h-10 w-11 place-items-center rounded-xl text-xl font-bold hover:bg-black/5" onClick={() => setTeamCount((count) => Math.max(2, count - 1))} aria-label="Færre lag">−</button><input type="number" min={2} max={Math.max(2, presentPlayers.length)} value={effectiveTeamCount} onChange={(event) => setTeamCount(Math.max(2, Number(event.target.value) || 2))} className="h-10 min-w-0 flex-1 bg-transparent text-center text-lg font-black outline-none" /><button type="button" className="grid h-10 w-11 place-items-center rounded-xl text-xl font-bold hover:bg-black/5" onClick={() => setTeamCount((count) => Math.min(Math.max(2, presentPlayers.length), count + 1))} aria-label="Flere lag">+</button></div>{suggestedGroups !== null && <span className="mt-2 block text-xs font-semibold leading-5 text-[var(--ink-soft)]">{suggestedGroups === effectiveTeamCount ? `Økten har en stasjonsbolk med ${suggestedGroups} stasjoner — ett lag på hver.` : `Økten har en stasjonsbolk med ${suggestedGroups} stasjoner. Med ${effectiveTeamCount} lag blir ikke alle stasjonene bemannet samtidig.`}</span>}</label>}
  ...
  return <AppShell immersive><div className="min-h-screen pb-16">
    ...
    <main className="mx-auto max-w-[1180px] px-4 pt-7 sm:px-8 sm:pt-10">
      ...
      {!players.length ? <div className="mt-7"><EmptyState icon={<UsersRound size={24} />} title="Importer spillerlisten først" body="Legg til Hoopit-listen under Lag og spillere, og gå deretter tilbake hit for å registrere oppmøte." action={<Link href="/team" className="inline-flex min-h-11 items-center rounded-xl bg-[var(--orange)] px-4 text-sm font-semibold text-white">Gå til Lag og spillere</Link>} /></div> : activeStep === "check-in" ? <section className="mt-7 rounded-[28px] border border-[var(--line)] bg-[var(--surface)] p-5 shadow-[0_10px_35px_rgba(16,32,29,.05)] sm:p-7">
...
export function WorkoutSession({ sessionId }: { sessionId: string }) {
  ...
  const schedule = session ? sessionSchedule(session) : null;
  ...
  return <AppShell immersive><div className="min-h-screen pb-28 sm:pb-16">
    ...
    <main ref={runnerTop} className="mx-auto max-w-[1080px] scroll-mt-24 px-4 pt-5 sm:px-8 sm:pt-8">
      {session.blocks.length ? <>
        ...
        {currentBlock && <section aria-labelledby="current-block-title" className="mt-4 overflow-hidden rounded-[26px] border border-[var(--line)] bg-[var(--surface)] shadow-[0_12px_40px_rgba(16,32,29,.07)]"><header className="flex items-center gap-3 border-b border-[var(--line)] bg-[var(--paper-deep)] px-4 py-4 sm:px-6 sm:py-5"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[var(--orange)] text-sm font-black text-white">{safeBlockIndex + 1}</span><div className="min-w-0 flex-1"><p className="text-[10px] font-black uppercase tracking-[.13em] text-[var(--ink-soft)]">{(() => { const clock = blockClock(schedule, currentBlock.id); return clock ? <span className="tabular-nums">{clockTime(clock.startsAt)}–{clockTime(clock.endsAt)}</span> : "Gjeldende bolk"; })()}</p><h2 id="current-block-title" className="truncate text-2xl font-black tracking-[-.04em] sm:text-3xl">{currentBlock.title}</h2></div>{isStationBlock(currentBlock) && currentBlock.items.length > 0 && <Tag tone="neutral">{currentBlock.items.length} × {stationRotation(currentBlock)} min</Tag>}<Tag tone="neutral">{blockDuration(currentBlock)} min</Tag></header>{isStationBlock(currentBlock) && <div className="flex items-center gap-2 border-b border-[var(--line)] bg-[var(--paper-deep)]/60 px-4 py-2.5 text-xs font-bold text-[var(--ink-soft)] sm:px-6"><LayoutGrid size={14} />Stasjonene går samtidig. {currentBlock.items.length > 0 ? `${currentBlock.items.length} grupper, rotasjon hvert ${stationRotation(currentBlock)}. minutt.` : ""}</div>}{currentBlock.notes && <div className="border-b border-[var(--line)] bg-[#f8f5ed] px-4 py-3 text-sm font-semibold leading-6 text-[var(--ink-soft)] sm:px-6"><span className="mr-2 text-[10px] font-black uppercase tracking-[.11em] text-[var(--orange)]">Notat for bolken</span>{currentBlock.notes}</div>}<div className={cn("grid gap-3 p-3 sm:p-5", isStationBlock(currentBlock) && "sm:grid-cols-2")}>{currentBlock.items.length ? currentBlock.items.map((item, itemIndex) => <button key={item.id} type="button" onClick={() => setSelectedItem(item)} className="group rounded-[20px] border border-[var(--line)] bg-white p-4 text-left transition hover:border-[var(--ink)] hover:shadow-sm sm:p-5"><div className="flex items-start gap-3"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[var(--paper-deep)] text-xs font-black">{itemIndex + 1}</span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><h3 className="text-lg font-black tracking-[-.025em]">{item.title}</h3>{!isStationBlock(currentBlock) && <span className="shrink-0 text-sm font-black text-[var(--ink-soft)]">{item.durationMinutes} min</span>}</div><ItemCoach item={item} members={sessionTeam?.members ?? []} />{item.description && <p className="mt-2 clamp-2 text-sm leading-6 text-[var(--ink-soft)]">{item.description}</p>}{item.coachingNotes && <div className="mt-3 rounded-xl bg-[#fff0e8] px-3.5 py-3"><p className="text-[10px] font-black uppercase tracking-[.11em] text-[#9c3913]">Stikkord</p><p className="mt-1 text-sm font-semibold leading-6">{item.coachingNotes}</p></div>}<span className="mt-3 inline-flex items-center gap-1.5 text-xs font-black text-[var(--orange)]"><CirclePlay size={15} />Vis øvelsen</span></div></div></button>) : <p className="py-8 text-center text-sm text-[var(--ink-soft)]">{isStationBlock(currentBlock) ? "Ingen stasjoner i denne bolken." : "Ingen aktiviteter i denne bolken."}</p>}</div></section>}
    ...
    <Modal open={overviewOpen} onClose={() => setOverviewOpen(false)} title="Øktoversikt" description={`${session.blocks.length} ${session.blocks.length === 1 ? "bolk" : "bolker"} · ${minutesLabel(session.plannedDurationMinutes)}`} size="lg">
      ...
      <div className="grid gap-3">{session.blocks.map((block, index) => <button key={block.id} type="button" onClick={() => showBlock(index)} aria-label={`Gå til ${block.title}`} className={cn("rounded-2xl border p-4 text-left transition hover:border-[var(--ink)]", index === safeBlockIndex ? "border-[#83ad9c] bg-[#e8f3eb]" : "border-[var(--line)] bg-white")}><div className="flex items-center gap-3"><span className={cn("grid h-9 w-9 shrink-0 place-items-center rounded-xl text-xs font-black", index === safeBlockIndex ? "bg-[#34745f] text-white" : "bg-[var(--paper-deep)]")}>{index + 1}</span><span className="min-w-0 flex-1"><strong className="block truncate">{block.title}</strong><small className="text-[var(--ink-soft)]">{isStationBlock(block) ? `${block.items.length} ${block.items.length === 1 ? "stasjon" : "stasjoner"} × ${stationRotation(block)} min` : `${block.items.length} ${block.items.length === 1 ? "aktivitet" : "aktiviteter"}`}</small></span><span className="shrink-0 text-sm font-black text-[var(--ink-soft)]">{blockDuration(block)} min</span><ChevronRight size={17} className="shrink-0 text-[var(--ink-soft)]" /></div>{block.items.length > 0 && <ul className="mt-3 grid gap-1.5 pl-12 text-xs text-[var(--ink-soft)]">{block.items.map((item, itemIndex) => { const coach = assignedCoach(item, sessionTeam?.members ?? []); return <li key={item.id} className="flex items-start justify-between gap-3"><span>{isStationBlock(block) && <span className="font-bold">{itemIndex + 1}. </span>}{item.title}{coach && <span className="font-bold"> · {coach.fullName}</span>}</span>{!isStationBlock(block) && <span className="shrink-0 font-bold">{item.durationMinutes} min</span>}</li>; })}</ul>}</button>)}</div>
...
/**
 * Who runs this activity in the hall. Nothing is drawn when nobody was named:
 * an activity is the whole coaching team's by default, and a "Ingen ansvarlig"
 * line on every card would bury the few that were handed to someone.
 */
function ItemCoach({ item, members }: { item: SessionItem; members: Profile[] }) {
  const coach = assignedCoach(item, members);
  if (!coach) return null;
  return <p className="mt-2 flex items-center gap-1.5"><Avatar name={coach.fullName} initials={coach.initials} color={coach.color} size="sm" /><span className="text-xs font-bold text-[var(--ink-soft)]">{coach.fullName} har denne</span></p>;
}
