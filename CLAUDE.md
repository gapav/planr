...
The same route serves `intent: "reset"`, which mints a `recovery` link for a coach who cannot get in — so an administrator resets a password from `/admin` rather than from the Supabase dashboard.
